  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 mt-3 text-xs text-ink-muted group-hover:text-accent transition-colors">
        <span>
          {doc_count === 1 ? t("kb.cardDocumentOne") : t("kb.cardDocuments", { count: doc_count })}
          {" · "}
          {conversation_count === 1 ? t("kb.cardChatOne") : t("kb.cardChats", { count: conversation_count })}
        </span>
      </div>
      {confirming ? (
        <div
        ...
        >
          <p className="mb-2 text-xs font-medium text-error">{t("kb.deleteTitle")}</p>
        ...
        <button
          onClick={handleDelete}
          aria-label={t("kb.deleteLabel")}
          title={t("kb.deleteLabel")}
