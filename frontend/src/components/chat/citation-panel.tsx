    ...
    return (
      <div className={cn("p-6", className)}>
        <div className="flex flex-col items-center justify-center h-full text-center space-y-3">
          ...
          <div>
            <p className="text-sm font-medium text-ink-muted">No sources yet</p>
            <p className="text-xs text-ink-muted/60 mt-1">Sources will appear here as the AI answers</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={cn("h-full flex flex-col", className)}>
      <div className="flex items-center justify-between p-4 border-b border-hairline">
        <h3 className="text-sm font-semibold text-ink">Sources ({citations.length})</h3>
        {onClose && (
          <button onClick={onClose} className="p-1 rounded-md hover:bg-canvas-soft transition-colors">
        ...
        {Object.entries(grouped).map(([docId, cites]) => (
          <div key={docId} className="space-y-1.5">
            <p className="text-xs font-medium text-ink-muted uppercase tracking-wider px-1">
              {cites[0]?.doc_filename || ("Document " + docId.slice(0, 8) + "…")}
            </p>
            {cites.map((cite) => {
              const isActive =
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs font-medium text-ink-soft">Chunk {cite.chunk_index}</span>
